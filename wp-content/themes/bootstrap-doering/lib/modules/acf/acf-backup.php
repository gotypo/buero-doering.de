...
function getKeyvisual() {
    ...
    if ((get_field('keyvisualvideo') || (get_field('keyvisualimage')))) {
        ...
        if((get_field('keyvisualvideo'))) {
            $out .= ' <video poster="'.get_field('keyvisualimage').'" muted autoplay loop>
...
function team_acf_block_render_callback( $block ) {
    ...
    if (is_admin() ) {
        ...
        $size = 'full';
        $out = '<div class="team waypoint-marker">';
        $out .= '<div class="row">';
        ...
        $out .= '</div>';
        $out .= '</div>';
        $out .= '<div class="col-xl-8 col-lg-7 col-md-6 offset-md-1 team-right">';
        if(get_field('email')) {
...
function youtubevideo_acf_block_render_callback( $block ) {
    ...
    if (is_admin() ) {
        ...
        $out = '<div class="youtube-video-element waypoint-marker mt-4 mb-4">';
        $out .= '<div class="row">';
...
function shortcut_acf_block_render_callback( $block ) {
    ...
    if (is_admin() ) {
        ...
        $out = '<div class="shortcuts waypoint-marker">';
        $out .= '<div class="row">';
        $out .= '<div class="col-md-8 offset-md-2">';
        $out .= '<h2>'.get_field('shortcutheader').'</h2>';
        $out .= '<p>'.get_field('shortcutdescription').'</p>';
...
function services_acf_block_render_callback( $block ) {
    ...
    if (is_admin() ) {
        ...
        $out = '<div class="services waypoint-marker">';
        $out .= '<h2>'.get_field('servicesheader').'</h2>';
        $out .= '<p>'.get_field('servicesdescription').'</p>';
        ...
        while( have_rows('serviceitems') ) : the_row();
            ...
            $out .= '<div class="item '.$amountOfCols.' animate__animated" data-animation="'.$animationDirection.'">';
...
//    $childCategories = get_categories( $args );
    ...
    if (is_admin() ) {
        ...
        $out = '<div id="news" class="element-content el-posts waypoint-marker">';
        $out .= '<h2>'.$header.'</h2>';
        $out .= '<p>'.$description.'</p>';
        ...
        if($showCategory == 1) {
            ...
            $out .= '<div class="form-group custom-select">
                        ...
                        <option value="buerodoering.gotypo.de/blog/" selected>Filtere die News nach Event</option>';
        ...
        if ($query->have_posts()) {
            ...
            while (($query->have_posts()) && ($index <= $amountOfPosts)) {
                ...
                if($query->post->post_excerpt != "") {
                    $out .= '<p class="excerpt">'.wp_trim_words($query->post->post_excerpt, 30, '...').'</p>';
                } else {
                    $out .= '<p>'.wp_trim_words($content, 30, '...').'</p>';
                }
...
function pressfilter_acf_block_render_callback( $block ) {
    ...
    if (is_admin() ) {
        ...
        // KATEGORIE FILTER
        $out = '<div id="category-filter" class="event-filter filterbar">';
        $out .= '<div class="form-group custom-select">
                    <select class="form-select form-control" onchange="location = this.value;"> 
                    <option selected>Wähle aus, Für Welches Event du dich interessierst…</option>';
...
add_action('acf/init', 'button_acf_init');
function button_acf_init() {
    if( function_exists('acf_register_block') ) {
        acf_register_block(array(
            'name'				=> 'linkbutton',
            'title'				=> __('Button'),
...
function linkbutton_acf_block_render_callback( $block ) {
    ...
    if (is_admin() ) {
        ...
        $out = '<div class="align-'.$align.'">';
        $out .= '<a href="'.$buttonLink["url"].'" class="btn" '.$target.'>'.$buttonText.'</a>';
        $out .= '</div>';
    }
...
function dividerstyle_acf_block_render_callback( $block ) {
    ...
    if (is_admin() ) {
        ...
        if($dividerstyle == 'triangle') {
            ...
            $out = '<div class="styled-divider element-full-width '.$dividerstyle.'"></div>';
            ...
            $out = '<div id="divider-with-image" class="divider-with-image element-full-width '.$dividerstyle.'">';
            $out .= '<div class="gradient-overlay"></div>';
            if((get_field('image'))) {
...
function slider_acf_block_render_callback( $block ) {
    ...
    $copryright = get_field('copryright');
    $size = 'medium';
    ...
    if($images) {
        ...
        if (is_admin() ) {
            ...
            if (is_array($images)) {
                $out = '<div id="'.$id.'" class="element element-contentswiper element-full-width">';

                if($sliderStyle == "sponsor") {
                    // SPONSOR SLIDER
                    $out .= '<div class="sponsorswiper swiper-container-horizontal">';
                    ...
                    // CONTENT SLIDER
                    $out .= '<div class="contentswiper swiper-container-horizontal">';
                    $out .= '<div class="swiper-wrapper">';

                    foreach ($images as $image) {
                        $out .= '<div class="swiper-slide"><img src="'. $image['sizes']['large'] .'" alt="" /></div>';
                    }
                ...
                if($copryright) {
                    $out .= '<div class="copyright"><span>&copy;</span>'.$copryright.'</div>';
                }
                $out .= '</div>';
            }
...
function pressinformation_acf_init() {
    ...
    if( function_exists('acf_register_block') ) {
        ...
        acf_register_block(array(
            ...
            'title'				=> __('Presseinformationen zu Event'),
            'description'		=> __(''),
...
function pressinformation_acf_block_render_callback( $block ) {
    ...
    if (is_admin() ) {
        ...
        $out = '<div id="'.$id.'" class="element-gutenberg waypoint-marker element-pressinformation">
					<div class="row">
...
function contactbox_acf_block_render_callback( $block ) {
    ...
    if (is_admin() ) {
        ...
        $out = '<div class="element-gutenberg waypoint-marker element-contact">
					<div class="row">
...
function gallery_acf_block_render_callback( $block ) {
    ...
    if($item) {
        ...
        if (is_admin() ) {
            ...
            if (is_array($item)) {
                $out = '<div class="element imagegallery">';
                $out .= '<div class="row">';
                foreach ($item as $k => $v) {
    ...
    if($artists) {
        ...
        if (is_admin() ) {
            ...
            if (is_array($artists)) {
                ...
                $out = '<div id="'.$id.'" class="element-gutenberg waypoint-marker element-lineup">';
...
function downloads_acf_block_render_callback( $block ) {
    ...
    if($downloads) {
        ...
        if (is_admin() ) {
            ...
            if (is_array($downloads)) {
                $out = '<div class="element-gutenberg waypoint-marker element-downloads">';
...
function hardfacts_acf_block_render_callback( $block ) {
    ...
    if(get_field('text')) {
        ...
        if (is_admin()) {
            ...
            $out = '<div id="'.$id.'" class="element-gutenberg waypoint-marker element-hard-facts">';
            $out .= '<div class="row">';
    ...
    if($accordeon) {
        ...
        if (is_admin()) {
            ...
            if (is_array($accordeon)) {
                ...
                $out = '<div class="element-gutenberg waypoint-marker element-collapsible">';
    ...
    if($schedule) {
        ...
        if (is_admin()) {
            ...
            if (is_array($schedule)) {
                ...
                $out = '<div class="element-gutenberg waypoint-marker element-schedule" id="'.$id.'">';
    ...
    if($counter) {
        ...
        if (is_admin() ) {
            ...
            if (is_array($counter)) {
                ...
                $out = '<div class="element-gutenberg waypoint-marker element-counter">';
                $out .= '<div class="row same-height-row">';
...
function article_acf_block_render_callback( $block ) {
    ...
    if (is_admin() ) {
        ...
        $out = '<div class="element-gutenberg waypoint-marker element-text-image element-text-'.$layout.'">';
