...
/*   blacklistening blogs   */
add_action( 'enqueue_block_editor_assets', 'my_plugin_blacklist_blocks' );
function my_plugin_blacklist_blocks() {
    wp_enqueue_script( 'my-plugin-blacklist-blocks', get_stylesheet_directory_uri() . '/assets/js/gutenberg.js', array( 'wp-blocks', 'wp-dom-ready', 'wp-edit-post' ));
}

/*   editor styles   */
add_action( 'enqueue_block_editor_assets', 'legit_block_editor_styles' );
function legit_block_editor_styles() {
        wp_enqueue_style('legit-editor-styles', get_theme_file_uri('/assets/css/backend-gutenberg.css'), false, '1.0', 'all');
}
